...
public class Yazlab1 {
    ...
    public void pars (String fisDokumu)
    {
    	int bayrak=0;
	    	...
	    	//	String fisDokumu = "MIGROS TICARET A.S-. DENİZLİ DEMOKRASI MEYDANI SATIS MAGAZASI SITELER MH. INCILIPINAR CD. NO:8A2/B TEL:0(850)3102659 — PAMUKKALE/DENİZLİ BUYUK MUKELLEFLER V.D.6220529513 http://www.migros.com.tr : MERKEZ ADRESİ: ATATÜRK MAH. TURGUT ÖZAL BULV. NO:7 ATAŞEHİR/İSTANBUL  MERSIİS NO: 0622052951300016  TARIH:  2D/02/2018 SAAT : 10:903  FİŞNO: İZb  #6002080107449672  ###86905705418164#  xx FINISH OVANTUM 80 %18 *39,90  HAHHHH##SO0ODO2Z7OA  xx ROCHER 3 LÜ X8 *3,O0  TOPKDV X18,2 - c  TOPLAM   33,40  444677xxxxxx2006 — ORTAK POS x43,40  KDV KDV TUTARI KDV'Lİ TOPLAM  x8 x*0,26 x3, 50  x18 x6,09 x39,90 KASiİYER DOo — CEYDA fft*'k*******X***t*******************XX***";
	    	...
	    	for(int i=0;i<parsedStr.length;i++)
	    	{
	    		...
	    		if(parsedStr[i].equals("TARİH")||parsedStr[i].equals("TARIH") || parsedStr[i].equals("TARİK")|| parsedStr[i].equals("TARIK")) {
	    			for (int j=1;j<10;j++)
	    			{
	    		...
	    		if(parsedStr[i].equals("FİŞNO")||parsedStr[i].equals("FİENO")) {
	    	...
	    	for(int i=0;i<parsedStr.length;i++)
	    	{
	    		...
	    		if(parsedStr[i].equals("FİŞ")) {
	    			if(parsedStr[i+1].equals("NO")) {
	    			//System.out.println("fiş:"+parsedStr[i]);
	    			for (int j=1;j<10;j++)
	    			{
	    				if(parsedStr[(i+1)+j].isEmpty())
	    			{
	    				...
	    				//System.out.println("Fiş NO: "+parsedStr[(i+1)+j]);
	    		...
	    		if(parsedStr[i].equals("FİŞNO")||parsedStr[i].equals("FİENO")||parsedStr[i].equals("FIŞNO") || parsedStr[i].equals("FISNO")) {
 	    				...
 	    				if(parsedStr[i+j].isEmpty())
 	    			{
 	    				...
 	    				//System.out.println("Fiş NO: "+parsedStr[i+j]);
              ...
              //  String sql_sorgu2="insert into urunler (FisNoID, urunler,kdv,fiyat) values ((SELECT fis.fisNoID FROM ((isletmeler INNER JOIN fis ON isletmeler.isletmeID = fis.isletmeID  ))where fisNo='"+parsedilen[2]+"' and İsletmeAdi='"+parsedilen[0]+"'  ),'"+parsedilen[4]+"','"+parsedilen[5]+"','"+parsedilen[6]+"'";
	    	...
	    	for(int i=0;i<parsedilen.length;i++) {
    ...
    public void listele()
    {
        ...
        try
        {
            while(myRs.next())
            {
//					satï¿½rlar[0]=myRs.getString("denemeId");
//					satï¿½rlar[1]=myRs.getString("name");
//					satï¿½rlar[2]=myRs.getString("Soyad");
//					satï¿½rlar[3]=myRs.getString("Tarih");
    ...
    private void initialize()
    {
            ...
            public void actionPerformed(ActionEvent arg0)
            {
                ...
                if(selected ==JFileChooser.APPROVE_OPTION)
                {
        		   ...
        		   Imgproc.resize(dst, dst2, size, 0, 0, Imgproc.INTER_AREA);
        		      ...
        		      Imgcodecs.imwrite("/Users/sadettinayyildiz/Desktop/denemeOpenCV.jpg", destination);                    


                    File imageFile = new File("/Users/sadettinayyildiz/Desktop/denemeOpenCV.jpg");
                    ITesseract instance = new Tesseract();
                    progressBar.setValue(100);
                    instance.setDatapath("/Users/sadettinayyildiz/eclipse-workspace/TessTest/tessdata");
        ...
        button_1.addActionListener(new ActionListener()
        {
            ...
            {
/*
            	String sql_sorgu="insert into isletmeler(isletmeAdi) values ('KİM')";
            	 String sql_sorgu1="insert into fis (isletmeID,tarih,fisNo,toplamFiyat) values ((select isletmeID from isletmeler where isletmeAdi = 'KİM'),'08-01-2019','8','11')";
            	 String sql_sorgu2="insert into urunler (FisNoID, urunler,kdv,fiyat) values ((SELECT fis.fisNoID FROM ((isletmeler INNER JOIN fis ON isletmeler.isletmeID = fis.isletmeID  ))where fisNo='8' and İsletmeAdi='A101' ),'Ekmek','21','13')";
               ...
               //    String sql_sorgu2="insert into urunler (FisNoID, urunler,kdv,fiyat) values ((SELECT fis.fisNoID FROM ((isletmeler INNER JOIN fis ON isletmeler.isletmeID = fis.isletmeID  ))where fisNo='"+parsedilen[2]+"' and İsletmeAdi='"+parsedilen[0]+"'  ),'"+parsedilen[4]+"','"+parsedilen[5]+"','"+parsedilen[6]+"'";
                ...
                baglanti.ekle(sql_sorgu1);
                for (int i = 0; i <parsedilen1.size(); i++) {
                 sql_sorgu2="insert into urunler (FisNoID, urunler,kdv,fiyat) values ((SELECT fis.fisNoID FROM ((isletmeler INNER JOIN fis ON isletmeler.isletmeID = fis.isletmeID  ))where fisNo='"+parsedilen[2]+"' and İsletmeAdi='"+parsedilen[0]+"'  ),'"+parsedilen1.get(i)+"','"+parsedilen2.get(i)+"','"+parsedilen3.get(i)+"')";
                     baglanti.ekle(sql_sorgu2);
				}
        ...
        String sıralama[]= {"isletmeID","İşletme Adı","Tarih","Fiş No","Toplam Fiyat","Ürünler","Kdv","Ürün Fiyat"};
        JComboBox comboBox = new JComboBox(sıralama);
        ...
        String sıralama2[]= {"Küçükten Büyüğe","Büyükten Küçükğe"};
        JComboBox comboBox_1 = new JComboBox(sıralama2);
